<div>
    ...
    <div class="card mt-5 p-4">
        <div class="flex items-center justify-between">
            <span class="text-xs text-white/45">Belanja sehari kalau semua hidup</span>
            <span class="bg-dyno-gradient bg-clip-text text-xl font-extrabold text-transparent">
                RM{{ intdiv($adSet->totalDailyBudgetSen(), 100) }}
            </span>
        </div>
        <div class="mt-2.5 flex items-start justify-between gap-4 border-t border-white/5 pt-2.5">
            <span class="text-xs text-white/45">Kawasan</span>
    ...
    <button type="button" wire:click="runAll"
            ...
            class="mt-4 w-full rounded-xl bg-gradient-to-r from-emerald-500 to-teal-400 px-4 py-5 text-lg font-extrabold text-ink-950 shadow-glow transition active:scale-[0.99] disabled:opacity-50"
        ...
        @foreach ($adSet->variants as $variant)
            @php
                $badge = match ($variant->status) {
                    'active' => ['bg-emerald-500/20 text-emerald-300', 'HIDUP'],
                    'failed' => ['bg-rose-500/20 text-rose-300', 'GAGAL'],
                    default => ['bg-white/10 text-white/55', 'PAUSED'],
                };
            @endphp
            <div class="card flex gap-3 p-3">
                <img src="{{ Storage::disk('public')->url($variant->image_path) }}" alt=""
                     class="h-[72px] w-[72px] shrink-0 rounded-xl border border-white/10 object-cover">
                <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-1 line-clamp-2 text-xs leading-relaxed text-white/45">{{ $variant->caption }}</p>

                    @if ($variant->last_error)
                        <p class="mt-1 text-xs text-rose-300">{{ $variant->last_error }}</p>
                    @endif
                        ...
                        <div class="mt-2.5">
                            @if ($variant->isLive())
                                <button type="button" wire:click="pauseVariant({{ $variant->id }})"
                                        class="rounded-lg border border-white/15 px-3 py-1.5 text-xs font-semibold text-white/80">Pause</button>
                            @else
                                <button type="button" wire:click="runVariant({{ $variant->id }})"
                                        class="rounded-lg border border-emerald-400/30 bg-emerald-500/15 px-3 py-1.5 text-xs font-semibold text-emerald-300">Hidupkan</button>
                            @endif
                        </div>
                    @endif
                </div>
